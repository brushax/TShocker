  ...
  (
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'fixed inset-y-0 z-10 hidden h-screen w-[var(--sidebar-width)] p-2 transition-[left,right,width] duration-200 ease-linear md:flex',
            side === 'left'
              ? 'left-0 rounded-r-2xl group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]'
              : 'right-0 rounded-l-2xl group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]',
            // Adjust the padding for floating and inset variants.
            variant === 'floating' || variant === 'inset'
              ? 'group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]'
              : 'group-data-[collapsible=icon]:w-[var(--sidebar-width-icon)]',
            className,
            'border border-[var(--color-sidebar-border)] bg-[var(--color-sidebar)]'
          )}
          {...props}
        >
          <div data-sidebar="sidebar" className="flex h-full w-full flex-col bg-transparent">
            {children}
          </div>
        </div>
      </div>
